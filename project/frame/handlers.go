...
var CSHandlerMap map[int32]HandlerFunc

func init()  {
	CSHandlerObj = &CSHandler{}
	CSHandlerMap = map[int32]HandlerFunc{
...
func (h *CSHandler) OnHandlerLogin(body []byte) bool {
	...
	LogDebug("Received protoId: 1, req: %v", req.String())
	if err := h.HandlerLogin(req); err != nil {
		LogError("Failed to handle protoId: 1, err: %v", err)
		return false
	}
	return true
}
...
func (h *CSHandler) OnHandlerCommon(body []byte) bool {
	...
	LogDebug("Received protoId: 2, req: %v", req.String())
	if err := h.HandlerCommon(req); err != nil {
		LogError("Failed to handle protoId: 2, err: %v", err)
		return false
	}
	return true
}
